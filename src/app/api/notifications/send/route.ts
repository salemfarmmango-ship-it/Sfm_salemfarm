...
export async function POST(request: NextRequest) {
    try {
        ...
        if (tokens && tokens.length > 0) {
            ...
            let url = '${process.env.NEXT_PUBLIC_API_URL}/notifications.php?action=list';
        ...
        if (failedTokens.length > 0) {
            await fetch('${process.env.NEXT_PUBLIC_API_URL}/notifications.php', {
                method: 'DELETE',
...
export async function GET(request: NextRequest) {
    try {
        const response = await fetch('${process.env.NEXT_PUBLIC_API_URL}/notifications.php?action=list');
