...
export default async function Home() {
    ...
    try {
        const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/products.php?is_featured=true', { cache: 'no-store' });
        if (res.ok) {
    ...
    try {
        const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/products.php?limit=12', { cache: 'no-store' });
        if (res.ok) {
            const data = await res.json();
    ...
    let blogs: any[] = [];
    try {
        const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/blogs.php?status=published', { cache: 'no-store' });
    ...
    let offers: any[] = [];
    try {
        const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/offers.php', { cache: 'no-store' });
    ...
    try {
        const res = await fetch('${process.env.NEXT_PUBLIC_API_URL}/public-settings.php', { cache: 'no-store' });
        if (res.ok) {
            const data = await res.json();
